...
function buildFullContext(
  ...
  lbs: LbsHotsellResult,
  promo: PromoMatchResult,
  crossSell: CrossSellResult
): string {
  const reprLines = replenishment.items.map(i =>
    `  · ${i.sku_name}：${i.urgency === 'urgent' ? '紧急' : '建议'}，${i.days_since_last}天未订（均期${i.avg_cycle_days}天），建议补${i.suggested_qty}箱`
  )
  ...
  const lbsLines =
    lbs.items.length === 0
      ...
      : lbs.items.map(
          i =>
            `  · ${i.sku_name}（${i.brand}）：周边${i.nearby_store_count}/${lbs.nearby_stores_analyzed}家在售，月均${i.avg_monthly_qty}箱`
        )

  const promoLines = promo.items.map(
    i =>
      `  · ${i.promo_name}${i.is_expiring_soon ? `（${i.expiry_days}天后到期）` : ''}：${i.description}，${i.gap_qty > 0 ? `还需加购${i.gap_qty}箱` : '已满足条件'}`
  )

  const crossLines = crossSell.items.map(
    i =>
      `  · ${i.sku_name}（${i.brand}）：置信度${i.confidence}%，${i.reason}`
  )
