import type { CrossSellResult, CrossSellItem } from '../types/index.js'

export function runCrossSell(storeId: string, ctx: ReturnType<typeof import('../data/loader.js').getDataContext>): CrossSellResult {
  ...
  for (const profile of storeProfiles) {
    ...
    for (const [targetSku, coCount] of associations.entries()) {
      if (storeSkuSet.has(targetSku)) continue // Already carries this
      ...
      if (confidence < 0.4 || lift < 1.0) continue
