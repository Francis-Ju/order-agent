...
function buildSkillContext(skill, result) {
    if (skill === 'replenishment') {
        const r = result;
        const lines = r.items.map(item => `  • ${item.sku_name}：${item.urgency === 'urgent' ? '紧急' : item.urgency === 'suggested' ? '建议' : '可选'}，已${item.days_since_last}天未订（均期${item.avg_cycle_days}天），建议补货${item.suggested_qty}箱`);
        return `补货预测结果：
...
${lines.join('\n')}`;
    ...
    if (skill === 'lbs_hotsell') {
        const r = result;
        const lines = r.items.map(item => `  • ${item.sku_name}（${item.brand}）：周边 ${item.nearby_store_count}/${r.nearby_stores_analyzed} 家在售，渗透率 ${Math.round(item.penetration_rate * 100)}%，月均 ${item.avg_monthly_qty} 箱`);
        return `周边热卖分析（分析了 ${r.nearby_stores_analyzed} 家3km内门店）：
${r.items.length === 0 ? '本店品类覆盖完整，无明显缺失' : lines.join('\n')}`;
    }
    if (skill === 'promo_match') {
        const r = result;
        const lines = r.items.map(item => `  • ${item.promo_name}${item.is_expiring_soon ? `（${item.expiry_days}天后到期！）` : ''}：${item.description}，${item.gap_qty > 0 ? `还需加购 ${item.gap_qty} 箱` : '已满足条件'}`);
        return `促销匹配结果（${r.expiring_soon_count} 个即将到期）：
${lines.join('\n')}`;
    }
    if (skill === 'cross_sell') {
        const r = result;
        const lines = r.items.map(item => `  • ${item.sku_name}（${item.brand}）：置信度 ${item.confidence}%，提升度 ${item.lift.toFixed(1)}x，${item.reason}`);
        return `关联推荐结果：
${lines.join('\n')}`;
    }
    return '';
...
skillChatRouter.post('/', async (req, res) => {
    const { store_id, skill, messages = [] } = req.body;
    ...
    try {
        ...
        let result;
        if (skill === 'replenishment')
            result = runReplenishment(store_id, ctx);
        else if (skill === 'lbs_hotsell')
            result = runLbsHotsell(store_id, ctx);
        else if (skill === 'promo_match')
            result = runPromoMatch(store_id, ctx);
        else if (skill === 'cross_sell')
            result = runCrossSell(store_id, ctx);
        else {
