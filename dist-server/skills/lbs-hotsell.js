...
export function runLbsHotsell(storeId, ctx) {
    const targetStore = ctx.stores.find(s => s.store_id === storeId);
    if (!targetStore)
        return { items: [], nearby_stores_analyzed: 0 };
    ...
    const thisStoreSkus = new Set((ctx.storeSkuProfiles.get(storeId) ?? []).map(p => p.sku_id));
    // Count how many nearby stores sell each SKU (that this store doesn't have)
    const nearbySkuCount = new Map();
    for (const nearbyStore of nearbyStores) {
        ...
        for (const profile of nearbyProfiles) {
            if (thisStoreSkus.has(profile.sku_id))
                continue;
    ...
    for (const [skuId, { count, totalQty }] of nearbySkuCount.entries()) {
        ...
        if (penetrationRate < 0.3)
            continue; // At least 30% of nearby stores
        items.push({
            sku_id: skuId,
